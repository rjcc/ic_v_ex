...
* Only cases with AGE_DON (Donor age) >= 50y
...
sts graph, failure ytitle(Proportion of Event) ///
    ...
    subtitle(Ischemia Time >= 3 Hours for Jan 2015 ~ Oct 2021)
...
use "$dirname/$dataname", clear
drop if FLUSH`n'==""
keep if AGE_DON>=50 | ISCHTIME>=4
local logname heartFLUSH`n'_age_don_50up_itime_4h+
...
* Grouping setting 6: FLUSH6: EX (CELSIOR & HTK) vs. IC (UW)
...
local logname heartFLUSH_age_don_50up_itime_4h+_los
log using `logname', smcl replace
* co3 FLUSH1 LOS
* co3 FLUSH2 LOS
* co3 FLUSH3 LOS
